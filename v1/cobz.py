...
class TextEditor:
    ...
    def _update_status(self) -> None:
        ...
        self.status_var.set(
            f"{filename} | Ln {cursor_line}, Col {int(cursor_column) + 1} | {characters} chars | {words} words | {wrap} | {dirty}"
        )

    def _update_title(self) -> None:
        name = str(self.current_file) if self.current_file else "Untitled"
        marker = "*" if self.is_dirty else ""
        self.root.title(f"{marker}{name} - Kat Lite")
    ...
    def open_file(self) -> bool:
        ...
        try:
            ...
            messagebox.showerror("Open Failed", "This editor currently supports UTF-8 text files.", parent=self.root)
            return False
    ...
    def show_about(self) -> None:
        messagebox.showinfo(
            "About Kat Lite",
            "Kat Lite\n\nA simple Linux-friendly text editor built with Python and tkinter.\n\nShortcuts:\nCtrl+N New\nCtrl+O Open\nCtrl+S Save\nCtrl+Shift+S Save As\nCtrl+F Find\nCtrl+Q Quit",
            parent=self.root,
        )
