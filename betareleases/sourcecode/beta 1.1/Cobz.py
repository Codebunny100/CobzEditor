...
class TextEditor:
    ...
    def find_text(self) -> None:
        ...
        found_any = False
        while True:
            index = document.text_area.search(search_term, index, stopindex="end", nocase=True)
            if not index:
                break
        ...
        if found_any:
            first_match = document.text_area.tag_nextrange("match", "1.0")
            if first_match:
                document.text_area.mark_set("insert", first_match[0])
                document.text_area.see(first_match[0])
        else:
